...
The Arsenale browser extension is a Chrome Manifest V3 extension (with Firefox compatibility via webextension-polyfill) that provides multi-account management for connecting to multiple Arsenale server instances. The extension includes a service worker that handles all API calls to Arsenale servers (bypassing CORS), a React popup with account switcher, vault status indicator, and tabbed sections for Keychain and Connections, and a React options/settings page for managing server accounts. Multi-account storage uses chrome.storage.local, with each account entry storing server URL, user identity, tokens, and vault status. Token refresh is handled automatically via chrome.alarms with per-account alarms scheduled 60 seconds before JWT expiry; if refresh fails (401), the account is marked as session-expired and the extension badge shows an alert.

The Keychain tab provides vault unlock and keychain browsing directly from the extension popup. When the vault is locked, users see an unlock form supporting password or MFA (TOTP, SMS) methods. Once unlocked, a searchable, filterable secret list displays all vault entries with type icons (LOGIN, SSH_KEY, CERTIFICATE, API_KEY, SECURE_NOTE), folder navigation, type filtering, and a favorites toggle. Clicking a secret opens a detail view with per-field copy buttons and masked/reveal toggles for sensitive fields (passwords, private keys, passphrases). Clipboard auto-clear writes an empty string after 30 seconds via chrome.alarms to prevent accidental pastes. Decrypted secret data is never persisted in chrome.storage and is only held in popup component memory. If the vault auto-locks server-side (TTL expiry), the extension detects the 403 response on the next API call and re-prompts for unlock.

Authentication supports the full server auth lifecycle: email+password login, MFA challenge handling (TOTP authenticator codes, SMS verification, WebAuthn security key -- though WebAuthn is not available in extension popup contexts), MFA setup redirection to the web UI, tenant switching for multi-tenant deployments, and logout with server-side token revocation. Since extensions operate from a different origin and cannot use httpOnly cookies, the server refresh and logout endpoints accept the refresh token from the request body as a fallback. CSRF validation is skipped for extension clients (identified by Bearer token with no cookies) since cross-site requests cannot reach chrome-extension:// origins. The build uses Vite with a multi-entry configuration producing a ready-to-load unpacked extension in the dist/ directory.
